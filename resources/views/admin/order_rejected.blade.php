        ...
        <div class="card table-card">
            <div class="card-header" style="padding: 12px 15px">
                <div class="d-sm-flex align-items-center justify-content-between">
                    <h5 class="mb-3 mb-sm-0">Rejected</h5>
                    <div>
...
{{-- Modal --}}
<div class="modal fade modal-animate" id="set-back-success" tabindex="-1" aria-labelledby="setBackRejectedLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="setBackRejectedLabel">Are you sure to set back as Success?</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body"></div>
            <div class="modal-footer">
                <button type="button" class="btn btn-light-success" id="confirm-success-btn"><i class="fas fa-check-circle"></i> Set back Success</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    $(document).ready(function() {
        ...
        var table = $('#data-table').DataTable({
            ...
            footer: false,
            ajax: {
                url: '{{ route('table_order_rejected') }}',
                type: 'GET',
                beforeSend: function() {
                    $('.top-loader').show();
                },
            ...
            columns: [
                ...
                },
                {
                    ...
                    searchable: true,
                    render: function(data) {
                        return `<span class="badge bg-light-danger fs-6" style="float:right">${'<i class="fas fa-ban"></i> ' + data}</span>`;
                    }
                }
            ],
            order: [[0, 'desc']],
            ...
            drawCallback: function(settings) {
                ...
                $('.card-header h5').text('Rejected - ' + totalEntries);
                $('#rejectedCount').text(totalEntries);
                ...
                success: function(response) {
                    ...
                    let detailsHtml = `
                        ...
                        <div class="offcanvas-body">
                            ...
                            <div class="offcanvas-order-details-list"><span class="offcanvas-order-details-list-1">Status</span> <span class="offcanvas-order-details-list-2"><span class="badge bg-light-danger fs-6">${'<i class="fas fa-ban"></i> ' + response.order.service_status}</span></span></div>
                        ...
                        <button type="button" class="btn btn-light-success rounded-0" id="successButton">Set it back as success</button>
                    `;
        ...
        // Set back Success + Modal Confirmation
        $(document).on('click', '#successButton', function() {
            const orderId = $('#order-details .offcanvas-title').text().split(': ')[1];

            $(this).hide(); // Hide the Set back Success button
            const loadingSpinner = `
                <button class="btn btn-light-success rounded-0" id="loadingSpinner" type="button" disabled style="background: #2ca87f; color:white; border:0;" >
                    <span class="spinner-border spinner-border-sm" role="status"></span>
                    Loading...
                </button>
            `;
            $('#order-details').append(loadingSpinner); // show loader Set back Success button
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $('#set-back-success .modal-body').html(`
                        <div style="margin-bottom:10px">${serviceTitle}</div>
                        ${orderInputs}
                    `);

                    $('#order-details').offcanvas('hide'); // hide offcanvas
                    $('#set-back-success').modal('show'); // show modal

                    $('#confirm-success-btn').off('click').on('click', function() {
                        $(this).hide(); // hide the confirm button on modal
                        const loadingButton = `
                            <button class="btn btn-light-success lh-1" type="button" style="background: #2ca87f; color:white;">
                                <span class="spinner-border spinner-border-sm" role="status"></span>
                                Loading...
                            </button>
                        `;
                        $('#set-back-success .modal-footer').append(loadingButton); // show the loader on confirm button
                        // attempt to update order status
                        $.ajax({
                            url: '/admin/set-back-success/' + orderId,
                            method: 'POST',
                            contentType: 'application/json',
                            data: JSON.stringify({ status: 'Success' }),
                            success: function(response) {
                                table.ajax.reload();
                                $('#set-back-success').modal('hide'); // after update order, hide modal
                            },
                            error: function(xhr, status, error) {
                                alert("There was an error updating the status: " + xhr.responseJSON.message);
                                // Remove the loading button and show the confirm button again
                                $('#set-back-success .modal-footer .btn-light-success.lh-1').remove(); // remove loader on modal
                                $('#confirm-success-btn').show(); // show confirm button on modal
                            },
                            complete: function() {
                ...
                complete: function() {
                    ...
                    $('#successButton').show(); // show set back reject button on offcanvas
                }
            });
        });
